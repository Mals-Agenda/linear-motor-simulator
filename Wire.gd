...
## Both paths must be set before _ready() runs (set them in the Inspector
## or via the .tscn).  The mesh is built once at scene start; if you need
## a dynamic wire, call rebuild() each frame.

@export var from_path:   NodePath = NodePath()
@export var to_path:     NodePath = NodePath()
@export var wire_radius: float    = 0.003
@export var wire_color:  Color    = Color(0.85, 0.70, 0.10)   # copper-yellow
...
func rebuild() -> void:
	...
	var dir := seg / len                      # unit vector A → B

	# Build cylinder mesh along the segment
	...
	# Orient the cylinder so its local Y axis (height) points along dir.
	# Use a stable "up" reference that avoids degeneracy when dir ≈ ±Y.
